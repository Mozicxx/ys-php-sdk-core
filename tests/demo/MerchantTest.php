...
class MerchantTest extends Base
{
    ...
    public function testRegister()
    {
        ...
        $mchPic00 = json_decode($response->data, true);

        $request = new UploadPictureRequest();
        $request->userCode = $userCode;  //用户号
        $request->picType = "19";  // 图片类型
        $request->token = $mchToken['token']; // toekn值D:\pic\jpg
        $request->filePath = "E:/tp/jpg/19.jpg"; // 本地图片地址
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic19 = json_decode($response->data, true);
        ...
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic30 = json_decode($response->data, true);

        $request = new UploadPictureRequest();
        $request->userCode = $userCode;  //用户号
        $request->picType = "31";  // 图片类型
        $request->token = $mchToken['token']; // toekn值D:\pic\jpg
        $request->filePath = "E:/tp/jpg/31.jpg"; // 本地图片地址
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic31 = json_decode($response->data, true);
        ...
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic33 = json_decode($response->data, true);

        $request = new UploadPictureRequest();
        $request->userCode = $userCode;  //用户号
        $request->picType = "34";  // 图片类型
        $request->token = $mchToken['token']; // toekn值D:\pic\jpg
        $request->filePath = "E:/tp/jpg/34.jpg"; // 本地图片地址
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic34 = json_decode($response->data, true);
        ...
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic35 = json_decode($response->data, true);
        ...
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic36 = json_decode($response->data, true);

        $request = new UploadPictureRequest();
        $request->userCode = $userCode;  //用户号
        $request->picType = "37";  // 图片类型
        $request->token = $mchToken['token']; // toekn值D:\pic\jpg
        $request->filePath = "E:/tp/jpg/37.jpg"; // 本地图片地址
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic37 = json_decode($response->data, true);
        ...
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic50 = json_decode($response->data, true);

        $request = new UploadPictureRequest();
        $request->userCode = $userCode;  //用户号
        $request->picType = "51";  // 图片类型
        $request->token = $mchToken['token']; // toekn值D:\pic\jpg
        $request->filePath = "E:/tp/jpg/51.jpg"; // 本地图片地址
        $request->filename = 'demo.jpg';  // 图片名称
        $response = Factory::mchClient()->mchClass()->uploadPicture($request);
        $mchPic51 = json_decode($response->data, true);
        ...
        $merchantRegistryRequest->mobile = ""; //通知手机号
        $merchantRegistryRequest->email = "";  //邮箱
        $merchantRegistryRequest->notifyType = "";  //通知类型
        $merchantRegistryRequest->custflag = ""; //客户类型（2：个人商户(小微商户)，3：个体商户，4：企业商户，5：供应商
        ...
        $merchantRegistryRequest->bankAccountName = "";  //结算户名
        $merchantRegistryRequest->bankAccountType = "11";  //银行账户类型
        $merchantRegistryRequest->bankProvinc = "广东省";  //银行所在省
        $merchantRegistryRequest->bankAccountNo = "";  //结算账号
        $merchantRegistryRequest->companyCerttype = "19";  //企业证照类型
        $merchantRegistryRequest->settleAccountType = "1";  //结算方式
        $merchantRegistryRequest->bankname = "";  //银行行名
        $merchantRegistryRequest->bankMobile = "";  //银行预留手机号
       $merchantRegistryRequest->picIds = $mchPic00['picId'] .','. $mchPic19['picId'].','. $mchPic30['picId'].','. $mchPic31['picId'].','. $mchPic33['picId'].','. $mchPic34['picId'].','. $mchPic35['picId'].','. $mchPic36['picId'].','. $mchPic50['picId'].','. $mchPic51['picId']; //图片ID集合：多个图片id，中间以逗号“，”分割，注意是对应图片上传接口返回的picId密文
      $response = Factory::mchClient()->mchClass()->channel1Report($merchantRegistryRequest);
        var_dump($response);
    }
